...
class PersonService {
  ...
  async create(personData, faceImageBuffer = null, faceImageFilename = null) {
    return this.withLock(async () => {
      // Load fresh data from file
      try {
        const data = await fs.readFile(this.dataPath, 'utf-8');
  ...
  async update(personId, personData, faceImageBuffer = null, faceImageFilename = null) {
    return this.withLock(async () => {
      // Load fresh data from file
      try {
        const data = await fs.readFile(this.dataPath, 'utf-8');
